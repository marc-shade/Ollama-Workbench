...
patch.dict('sys.modules', ...) block).  The old import dance removed the
freshly imported ollama_workbench.* modules from sys.modules when the block
exited, so every @patch('ollama_workbench.knowledge.web_to_corpus.X')
decorator re-imported and patched a *different* module copy than the one the
tests called into, and no patch ever took effect.

No test here may launch a real browser, download a chromedriver, or touch the
network: crawler construction always goes through build_crawler(), which mocks
ChromeDriverManager/Service/webdriver.Chrome, and every fetch path is mocked
at the requests/selenium boundary.
"""
...
WTC = 'ollama_workbench.knowledge.web_to_corpus'


def build_crawler(url, output_format, temp_dir=None):
    """Construct a WebsiteCrawler without launching Chrome, downloading a
    chromedriver, or creating a temp dir inside the package folder.

    The returned crawler has a MagicMock driver (tests that need a specific
    driver overwrite crawler.driver themselves).
    """
    if temp_dir is None:
        temp_dir = tempfile.mkdtemp()
    with patch(f'{WTC}.ChromeDriverManager'), \
         patch(f'{WTC}.Service'), \
         patch(f'{WTC}.webdriver.Chrome'), \
         patch(f'{WTC}.get_random_user_agent', return_value='test-agent'), \
         patch(f'{WTC}.tempfile.mkdtemp', return_value=temp_dir):
        return WebsiteCrawler(url, output_format)


class TestUserAgentUtils(TestCase):
    """Test user agent utility functions"""

    @patch(f'{WTC}.UserAgent')
...
class TestWebsiteCrawlerInit(TestCase):
    ...
    @patch(f'{WTC}.tempfile.mkdtemp')
    @patch(f'{WTC}.webdriver.Chrome')
    @patch(f'{WTC}.ChromeDriverManager')
    @patch(f'{WTC}.Service')
    @patch(f'{WTC}.Options')
    @patch(f'{WTC}.get_random_user_agent')
    ...
    def test_crawler_init_chrome_failure(self, mock_warning, mock_chrome, mock_service,
                                        mock_driver_manager, mock_ua, mock_mkdtemp):
...
class TestWebsiteCrawlerFetching(TestCase):
    ...
    def setUp(self):
        """Set up test crawler"""
        self.crawler = build_crawler("https://example.com", "TXT")
        self.crawler.driver = None

    @patch(f'{WTC}.requests.get')
    @patch(f'{WTC}.get_random_user_agent')
    ...
    @patch(f'{WTC}.time.sleep')
    @patch(f'{WTC}.random.uniform')
...
class TestWebsiteCrawlerLinkDiscovery(TestCase):
    ...
    def test_find_links_on_page_valid_links(self, mock_urldefrag, mock_urljoin, mock_soup_class):
        ...
        # Mock link elements (MagicMock so link['href'] subscripting works)
        ...
        with patch.object(self.crawler, 'is_valid_url') as mock_is_valid:
            ...
            # find_links_on_page only ADDS links; the root URL stays queued
            # until crawl() pops it, so it is still in to_visit_links here.
            expected_links = {
                "https://example.com",
                "https://example.com/page1",
                "https://example.com/page2",
    ...
    def test_find_links_on_page_invalid_links(self, mock_soup_class):
        ...
        with patch(f'{WTC}.urljoin') as mock_urljoin:
            mock_urljoin.return_value = "https://other-domain.com/page"

            with patch(f'{WTC}.urldefrag') as mock_urldefrag:
...
class TestWebsiteCrawlerFileSaving(TestCase):
    ...
    def setUp(self):
        ...
        self.temp_dir = tempfile.mkdtemp()
        self.crawler = build_crawler("https://example.com", "PDF", temp_dir=self.temp_dir)

    def tearDown(self):
        """Clean up temp directory"""
        shutil.rmtree(self.temp_dir, ignore_errors=True)

    @patch(f'{WTC}.os.path.exists')
    @patch(f'{WTC}.os.makedirs')
    @patch(f'{WTC}.urlparse')
    def test_get_filename_basic(self, mock_urlparse, mock_makedirs, mock_exists):
        ...
        with patch(f'{WTC}.SCRIPT_DIR', '/script'):
    ...
    @patch(f'{WTC}.pdfkit.from_string')
...
class TestWebsiteCrawlerOutputGeneration(TestCase):
    ...
    def setUp(self):
        ...
        self.temp_dir = tempfile.mkdtemp()
        self.crawler = build_crawler("https://example.com", "TXT", temp_dir=self.temp_dir)
            ...
            with patch.object(self.crawler, 'fetch_page_selenium') as mock_selenium:
                ...
                with patch.object(self.crawler, 'extract_main_content') as mock_extract:
                    ...
                    with patch.object(self.crawler, 'find_links_on_page'):
                        with patch(f'{WTC}.time.sleep'):
                            with patch(f'{WTC}.random.uniform'):
                                self.crawler.crawl()
            ...
            with patch.object(self.crawler, 'fetch_page_selenium') as mock_selenium:
                ...
                with patch(f'{WTC}.time.sleep'):
                    with patch(f'{WTC}.random.uniform'):
                        ...
                        # A URL whose fetch fails on both transports is popped
                        # from the queue but NOT marked visited (so it stays
                        # eligible for a retry if another page links to it
                        # again), and nothing is added to crawled_data.
                        self.assertEqual(self.crawler.visited_links, set())
                        self.assertEqual(self.crawler.to_visit_links, set())
...
class TestMainStreamlitApp(TestCase):
    ...
    def test_main_app_ui_creation(self, mock_button, mock_selectbox, mock_text_input,
        ...
        # Columns are used as context managers -> MagicMock
    ...
    def test_main_app_crawling_flow(self, mock_urlparse, mock_file, mock_download,
        ...
        # Mock crawler
        with patch(f'{WTC}.WebsiteCrawler') as mock_crawler_class:
            mock_crawler = Mock()
            mock_crawler_class.return_value = mock_crawler

            with patch(f'{WTC}.os.path.join') as mock_join:
                mock_join.return_value = "/test/example.com.json"

                with patch(f'{WTC}.os.path.exists') as mock_exists:
                    mock_exists.return_value = True
...
class TestIntegrationScenarios(TestCase):
    ...
    def test_crawler_full_workflow_txt(self):
        ...
        crawler = build_crawler("https://example.com", "TXT")
        ...
        with patch.object(crawler, 'fetch_page') as mock_fetch:
            ...
            with patch.object(crawler, 'extract_main_content') as mock_extract:
                ...
                with patch(f'{WTC}.BeautifulSoup') as mock_soup_class:
                    # Mock link discovery
                    mock_soup = Mock()
                    mock_link = MagicMock()
                    ...
                    # Have the discovered link resolve back to the root URL:
                    # after the first (and only) iteration the root is already
                    # in visited_links, so nothing new is queued and the crawl
                    # stops after exactly one page.
                    with patch(f'{WTC}.urljoin') as mock_urljoin:
                        mock_urljoin.return_value = "https://example.com"

                        with patch(f'{WTC}.urldefrag') as mock_urldefrag:
                            mock_urldefrag.return_value = ("https://example.com", "")
    ...
    def test_error_handling_resilience(self):
        """Test crawler resilience to fetch-layer errors.

        Errors are injected at the requests.get boundary (the real seam):
        fetch_page must swallow them and return None, crawl() then falls back
        to Selenium (no driver here -> also None) and skips the page instead
        of crashing.
        """
        crawler = build_crawler("https://example.com", "PDF")
        crawler.driver = None
        ...
        for error in error_scenarios:
            with patch(f'{WTC}.requests.get') as mock_get:
                mock_get.side_effect = error

                with patch(f'{WTC}.get_random_user_agent', return_value='test-agent'):
                    with patch('streamlit.progress'), patch('streamlit.empty'), \
                        ...
                        with patch(f'{WTC}.time.sleep'):
                            with patch(f'{WTC}.random.uniform'):
                                ...
                                # The failing page is skipped, not recorded
                                self.assertEqual(crawler.crawled_data, [])

    def test_memory_and_resource_management(self):
        """Test queue draining for large crawls"""
        crawler = build_crawler("https://example.com", "TXT")
        crawler.driver = None
            ...
            with patch('streamlit.progress'), patch('streamlit.empty'), \
                ...
                with patch(f'{WTC}.time.sleep'):
                    with patch(f'{WTC}.random.uniform'):
                        ...
                        # Every queued URL was popped and processed. Failed
                        # fetches are not marked visited (retry semantics),
                        # so the queue drains to empty and nothing is
                        # recorded as visited or crawled.
                        self.assertEqual(len(crawler.to_visit_links), 0)
                        self.assertEqual(len(crawler.visited_links), 0)
                        self.assertEqual(len(crawler.crawled_data), 0)
