...
def initialize_session_state():
    ...
    if "agent_type" not in st.session_state:
        ...
        logger.info("Initialized agent_type to None")
    
    if "metacognitive_type" not in st.session_state:
        st.session_state.metacognitive_type = "None"
        logger.info("Initialized metacognitive_type to None")
    
    if "voice_type" not in st.session_state:
        st.session_state.voice_type = "None"
        logger.info("Initialized voice_type to None")
    
    # Initialize generation settings
    if "temperature" not in st.session_state:
        st.session_state.temperature = 0.7
        logger.info("Initialized temperature to 0.7")
    
    if "max_tokens" not in st.session_state:
        st.session_state.max_tokens = 4000
        logger.info("Initialized max_tokens to 4000")
    
    if "presence_penalty" not in st.session_state:
        st.session_state.presence_penalty = 0.0
        logger.info("Initialized presence_penalty to 0.0")
    
    if "frequency_penalty" not in st.session_state:
        st.session_state.frequency_penalty = 0.0
        logger.info("Initialized frequency_penalty to 0.0")
    
    # Initialize corpus settings
    if "selected_corpus" not in st.session_state:
        st.session_state.selected_corpus = "None"
        logger.info("Initialized selected_corpus to None")
    
    # Initialize UI state
    if "settings_expanded" not in st.session_state:
        st.session_state.settings_expanded = False
        logger.info("Initialized settings_expanded to False")
    
    if "total_tokens" not in st.session_state:
        st.session_state.total_tokens = 0
        logger.info("Initialized total_tokens to 0")
    ...
    if "enhanced_mode" not in st.session_state:
        ...
        logger.info("Initialized enhanced_mode to True")
    ...
    try:
        if os.path.exists(SETTINGS_FILE):
            ...
            for key, value in settings.items():
                ...
                logger.info(f"Loaded setting {key}: {value}")
            
            # Ensure model variables are synchronized
            if "selected_model" in st.session_state:
                st.session_state.current_model = st.session_state.selected_model
                logger.info(f"Synchronized current_model to {st.session_state.selected_model}")
...
def synchronize_model_variables():
    ...
    if "selected_model" in st.session_state and "current_model" in st.session_state:
        ...
        st.session_state.current_model = st.session_state.selected_model
        logger.info(f"Set current_model to {st.session_state.selected_model}")
    elif "current_model" in st.session_state and "selected_model" not in st.session_state:
        st.session_state.selected_model = st.session_state.current_model
        logger.info(f"Set selected_model to {st.session_state.current_model}")
...
def get_agent_prompt():
    ...
    try:
        ...
        logger.info(f"Constructing agent prompt with: {agent_type}, {metacog_type}, {voice_type}")
